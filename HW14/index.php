...
<html>
   ...
   <body>
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
         if (isset($_POST['base'])) {
            $base = $_POST['base'];
            setcookie("base", $_POST['base'], time() + (86400 * 30));
        }
        if (isset($_POST['background'])) {
         $background = $_POST['background'];
         setcookie("bg", $_POST['background'], time() + (86400 * 30));
     }
      }
      echo  "<style>
               table, th, td {
      ...
      ?>
      <h1>HW14 - Cookies - Joel Peckham</h1>
      <form action="index.php" method="POST">
      <label for="background">Background Color: </label>
         <select name="background" onchange="this.form.submit()">
            <?php
